...
const Carousel = ({ children, visilbleItems, infinity, restartOnEnd }) => {
  ...
  const prevHandler = event => {
    if (currentSlide > 0) {
      ...
      listRef.current.style.left = `-${(itemWidth.fullWidth +
        itemWidth.margin) *
        leftMultiplier *
        cloneQuant}px`;
      ...
      listRef.current.style.transform = `translate3d(${leftMultiplier *
        100}%, 0, 0)`;
      setCurrentSlide(currentSlide - 1);
  ...
  const nextHandler = event => {
    if (infinity && !restartOnEnd) {
      ...
      const currentItemsEls = listRef.current.querySelectorAll(
        ".carousel__item"
      );
      const listWidth =
      ...
      if (currentCount === currentSlide + 1) {
        Array.from(slideItemsEl).forEach(elNode => {
          const currentEls = listRef.current.querySelectorAll(
            ".carousel__item"
          );
