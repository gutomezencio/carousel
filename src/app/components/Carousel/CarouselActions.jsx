import React, { useImperativeHandle, useContext, useCallback, useEffect, useRef } from 'react'
  ...
  const goToSlide = useCallback(
    slideNumber => {
      ...
      if (!infinity && insideNumber >= 0 && insideNumber <= state.slideCount) {
        listRefCurrent.style.transform = `translate3d(-${insideNumber * 100}%, 0, 0)`
        dispatch({
          type: 'SET_CURRENT_SLIDE',
    ...
    [infinity, state.slideCount, listRefCurrent.style, dispatch]
  )
